...
export function useLocalTrips() {
  ...
  const addTrip = (entry: string, exit: string, country?: string, note?: string) => {
    const trip: Trip = { id: crypto.randomUUID(), entry, exit, country, note };
  ...
  const updateTrip = (id: string, entry: string, exit: string, country?: string, note?: string) => {
    setTrips((prev) => prev.map((t) => (t.id === id ? { ...t, entry, exit, country, note } : t)));
  };

  const exportTrips = (): string => {
    return JSON.stringify(
      { app: "schengen-days-calculator", version: 1, exportedAt: new Date().toISOString(), trips },
      null,
      2
    );
  };
